    ...
    python3 manage.py shell -c "from django.contrib.auth.models import User; u = User.objects.create_superuser('poop', 'admin@example.com', '.');from rest_framework.authtoken.models import Token; Token.objects.create(user=u); from sportscred.models import Profile; Profile.objects.create(user=u);"
